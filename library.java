...
public class library extends JFrame  {
	...
	library () { 
		...
		JButton viewItem = new JButton ("View Book");
	 ...
	 viewItem.setBounds(700,260, 100, 40);
	  viewAll.setBounds(700,320, 100,40);
	   ...
	   /*
	    * 
	    * Where clearAll is clicked  it removes the current text on display and replaces it with an empty string
	    */
	   ...
	   /*
	    * 
	    * When the search button is clicked we call the search function from the backend class
	    * which searches for the target ID and adds it a list and then returns the list 
	    * afterwards it replaces the current list with the setModel method and prints out 
	    ...
	    */
		search.addActionListener(new ActionListener() { 
			public void actionPerformed(ActionEvent e) { 	
				...
				try {
					listOfBooks.setModel(LibraryBackEnd.search(Integer.parseInt (idField.getText()), authorField.getText(), titleField.getText(), Integer.parseInt(qtyField.getText())));
		...
		/*
		 * When the viewBook button is clicked it splits the item in the list up into an array 
		 * and inserts the text separately back into the textfield so that the user can make updates 
		 ...
		 * the delete function is called from the backend class which takes the separate values 
		...
		delete.addActionListener(new ActionListener() { 
			public void actionPerformed(ActionEvent e) { 
				...
				try {
					LibraryBackEnd.delete(Integer.parseInt(stringArray[0]),stringArray[1],stringArray[2],Integer.parseInt(stringArray[3]));
		...
		/*
		 * 
		 * The update button requires the user to first click on the view book button to insert 
		 * the values of the book into the textfield. Then the user can proceed to make changes 
		 * and then press update which calls the update method from the backend class, gets the values 
		...
		update.addActionListener(new ActionListener() { 
			public void actionPerformed(ActionEvent e) { 
				...
				try {
						LibraryBackEnd.update(Integer.parseInt(idField.getText()),authorField.getText(),titleField.getText(),Integer.parseInt(qtyField.getText()),Integer.parseInt(idField.getText()));
						listOfBooks.setModel(LibraryBackEnd.viewAll());
		...
		/*
		 * This button adds a new book into the database. The method returns a list and then the list gets updated 
		...
		add.addActionListener(new ActionListener() { 
			public void actionPerformed(ActionEvent e) { 
				...
				try {
					LibraryBackEnd.add(Integer.parseInt(idField.getText()),authorField.getText(),titleField.getText(),Integer.parseInt(qtyField.getText()));
					listOfBooks.setModel(LibraryBackEnd.viewAll());
				} catch (SQLException e1) {
		...
		/*
		 * 
		 * Here we add all the labels, buttons and textfields to the mainframe of the programam
		 * 
		 * 
		 */
	...
	public static void main(String[] args) { 
		...
		try { 
			...
			Connection database = DriverManager.getConnection("jdbc:mysql://localhost:3306", "myuser", "12345");
